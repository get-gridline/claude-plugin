...
ALWAYS_ASK: dict[str, str] = {
    ...
    "create_webhook": (
        "arranges for a permanent copy of this project's operational events to be sent to an "
        "address, which is a data-egress decision rather than a configuration change. It "
        "prints no signing secret -- generating one is a separate act, in the dashboard or "
        "under `gridline apply`"
    ),
    "archive_task": "archives the task. Its history stays, but it stops accepting new sessions",
}

# **Claude Code namespaces a plugin's MCP tools; it does not call them by their bare name.** A
# plugin declaring an MCP server named `gridline` inside a plugin also named `gridline` is called
# as `mcp__plugin_gridline_gridline__<tool>`, not `mcp__gridline__<tool>` -- doubled because the
# plugin id and the server id both happen to be "gridline". `plugin.json` and `.mcp.json` pin both
# halves, so this is settled rather than guessed. The bare form is matched too, since nothing here
# is told which name a future Claude Code version or a differently-configured client will use, and
# matching a name too narrowly is silent: every case below that names `ALWAYS_ASK` would still
# read as untriggered rather than as a hook that stopped seeing the tool it was written for.
GRIDLINE_TOOL = re.compile(r"^mcp__(?:plugin_gridline_)?gridline__")
...
def decide(payload: dict) -> tuple[str, str]:
    ...
    match = GRIDLINE_TOOL.match(tool)
    if match:
        bare = tool[match.end():]
...
CASES: list[tuple[str, str, dict]] = [
    ...
    # **Namespaced the way Claude Code actually calls a plugin's own MCP server.** A case written
    # against a prefix `decide()` is never handed for real asserts nothing at all: it passes
    # whatever the guard does, so every confirmation below can be inert while this file goes on
    # reporting itself healthy.
    ("ask", "delete_tunnel",
     {"tool_name": "mcp__plugin_gridline_gridline__delete_tunnel", "tool_input": {"name": "acme"}}),
    ("ask", "delete_webhook",
     {"tool_name": "mcp__plugin_gridline_gridline__delete_webhook", "tool_input": {"endpoint": "e"}}),
    ("ask", "retire_tunnel_key",
     {"tool_name": "mcp__plugin_gridline_gridline__retire_tunnel_key",
      "tool_input": {"name": "a", "slot": "1"}}),
    ("ask", "retire_webhook_secret",
     {"tool_name": "mcp__plugin_gridline_gridline__retire_webhook_secret", "tool_input": {}}),
    ("ask", "revoke_invitation",
     {"tool_name": "mcp__plugin_gridline_gridline__revoke_invitation", "tool_input": {}}),
    ("ask", "rename_project",
     {"tool_name": "mcp__plugin_gridline_gridline__rename_project", "tool_input": {"name": "a"}}),
    ("ask", "replay_webhooks",
     {"tool_name": "mcp__plugin_gridline_gridline__replay_webhooks", "tool_input": {}}),
    ("ask", "archive_task",
     {"tool_name": "mcp__plugin_gridline_gridline__archive_task", "tool_input": {}}),
    ("ask", "create_webhook arranges data egress",
     {"tool_name": "mcp__plugin_gridline_gridline__create_webhook", "tool_input": {"url": "https://x"}}),
    # **The bare form is a deliberate second case, not a leftover.** Nothing here is told which
    # name a future Claude Code version or a differently-configured client will use, so both
    # spellings must keep working rather than only the one observed today.
    ("ask", "delete_tunnel, bare (unnamespaced) form",
    ...
    ("", "an ordinary write tool, namespaced",
     {"tool_name": "mcp__plugin_gridline_gridline__create_agent", "tool_input": {"id": "chat"}}),
    ("", "a read tool, namespaced",
     {"tool_name": "mcp__plugin_gridline_gridline__list_projects", "tool_input": {}}),
