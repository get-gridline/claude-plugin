...
CASES: list[tuple[str, str, dict]] = [
    ...
    # **Namespaced the way Claude Code actually calls it.** Before this fix these all used the
    # bare `mcp__gridline__...` form, which is not a name Claude Code ever sends for a plugin's
    # own MCP server -- so the selftest passed by asserting against a prefix `decide()` would
    # never see for real, and every one of these confirmations was inert in production.
    ("ask", "delete_tunnel",
    ...
    ("ask", "rotate_webhook_secret prints a secret",
     {"tool_name": "mcp__plugin_gridline_gridline__rotate_webhook_secret", "tool_input": {}}),
