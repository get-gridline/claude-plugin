...
attach_sandbox(project, harness_id="support-tools", connection="e2b-prod")
add_mcp_server(project, harness_id="support-tools", connection="wiki",
               tools=[{"name": "search", "description": "Full text search over the wiki"}])
create_agent(project, agent_id="support-triage", system="…",
             models=["anthropic:claude-sonnet-4-5"], harness="support-tools")
```
...
**That list is not a fence.** Tools are discovered from your server when the turn runs, so one you did
not list is still offered and still runs — deliberately, since a server that gains a tool between
deploys should be a new capability rather than a broken agent. What the list buys is a description the
model reads and a name you can hang a policy on. To scope an agent to part of a server, set the
harness's `permissions` — see `gridline-approvals`. Nothing else narrows the surface.
