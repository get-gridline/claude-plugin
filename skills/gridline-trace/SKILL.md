...
It is a **response header** rather than a field on the reply — `references/headers.md` names it. From
the SDK, read it off the response, or off a `GridlineError`, which carries `.request_id` so a failed
turn is traceable at all. It is also accepted **inbound** — if the caller sent their own, that is the
id, which is what lets their logs and Gridline's share one.
...
|---|---|
| `X-Route-Model` | Which model actually answered — not necessarily the first hop |
| `X-Route-Degraded` | A fallback answered, so output may differ |
| `X-Route-Lost` | What that hop could not do |
