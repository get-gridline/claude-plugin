...
Six capabilities, and **never the same key for two jobs**. A production key that leaks can spend
tokens: bad, bounded, and visible in your telemetry. If that same key could edit routing, whoever
held it could repoint a model and read every prompt.
...
- **`config`** — what this plugin asks for. **Reads** agents, harnesses and connections, and asks
  what a change would do. It does not write them, which surprises people: applying a whole project
  document is `deploy`, and moving a single agent, harness or connection is `admin`.
- **`content`** — reads a conversation, which is your customers' data. Its own capability so that
  debugging where a turn routed does not hand over every transcript beside it.
- **`admin`** — credentials, webhooks, tunnels, project creation.

`route`, `config`, `telemetry`, `content`, `deploy` and `admin` are the six, and wider implies
narrower: `admin` implies `deploy`, and `deploy` implies `config`. `gridline-keys` has the table.
