...
| `STATE` | thinking blocks, reasoning items | **Must be the same on every hop.** Opaque conversation state cannot transfer between vendors |

The consequence people hit: extended thinking is a `STATE` capability, and Anthropic's thinking
blocks and OpenAI's reasoning items are *different* state formats. **So an agent using extended
thinking cannot have a cross-vendor fallback.** That is a product constraint, not a bug — you pick
thinking, or a cross-vendor safety net.

`degraded` means *the output may differ*, because a different model answered. It deliberately does
not mean "lost a capability": losing prompt caching costs money but produces identical output, and
that is reported separately as `lost_capabilities`. Conflating them would make the most valuable
fallback look like a downgrade.

## Validation happens when you compile, not when you save

Half-finished configuration saves fine — otherwise editing would be unusable. Validity is enforced
when a snapshot is compiled for the data plane, because that is the boundary an invalid plan must
not cross. `check(project)` is how you ask before then, and `explain_agent` shows what a given
