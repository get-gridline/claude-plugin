...
the eight dimensions above, or `metadata.<key>` for one of your own session dimensions; `days` is a
whole number, 1 to 365, counted back from now. There is no named period — translate "last week" into
`days=7` yourself, and say which window you used.
...
## Grouping by a task, and by an experiment arm

**Never `task` itself** — it is not one of the eight. Group on session metadata instead, which `by`
does take: set the same value as a metadata key and ask for `by="metadata.task"`. It reports on the
label you set rather than on the task object, so say which of the two you answered.

A traffic split labels every conversation with its arm under the reserved key
`metadata.gridline.arm`, so `by="metadata.gridline.arm"` is spend per branch of the experiment.
**That is not spend per model**: each arm keeps its serving agent's own fallbacks beneath it, so a
failover inside an arm serves a model belonging to no arm and still bills to that arm's row. Read
`by="model"` beside it. See `tasks-and-metadata.md`.
