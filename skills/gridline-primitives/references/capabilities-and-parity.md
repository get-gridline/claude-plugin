...
**`capability` is one name, not a list.** Two requirements are two calls and the intersection is
yours to take. Then `explain_agent` for what a specific chain gives up, which answers the same
question for a chain that already exists and does the intersecting for you.
