...
To get the content, fetch it with a credential that is allowed to: `grid.approvals(...)` for a
pending approval, `GET /sessions/{id}/messages` for a transcript. The event tells you *that*
