...
The arguments a call is about are fetched from the control plane rather than from this client:
...
**Two halves, and they are easy to conflate.** The *client* is `gridline.control()`, which reads
`GRIDLINE_CONTROL_URL` / `GRIDLINE_CONFIG_KEY` like every other control-plane call. The
*capability* that key has to carry is **`content`**, not `config`: this reads the paused turn out
of the stored conversation, so it is the same credential `GET /sessions/{id}/messages` needs. A
`config`-only key reaches the right plane and is refused `403`.
