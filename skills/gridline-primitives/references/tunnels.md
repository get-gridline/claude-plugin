...
Then issue a credential — **from the dashboard.** It returns three secrets, once. Then run the
client:
...
| `GRIDLINE_INJECTION_KEY` | Only for a server authorised from a **vault entry**. Without it those calls are refused by name — never served as the whole tunnel |
| `GRIDLINE_SERVER_URL_<NAME>` | One per server. **The only place your addresses exist** |
