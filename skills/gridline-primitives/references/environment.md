...
| `GRIDLINE_SERVER_URL_<NAME>` | yes | One per server. The only place your addresses exist |
...
| `GRIDLINE_SERVER_TOKEN_<NAME>` | no | A bearer for that server. Sent to it and to nothing else |
...
One variable per server, and one more if it needs a credential. The suffix is the server's
name uppercased, with `-` and `.` folded to `_`:

```
GRIDLINE_SERVER_URL_WIKI=http://wiki.internal:8000/mcp
GRIDLINE_SERVER_TOKEN_WIKI=…

GRIDLINE_SERVER_URL_JIRA=https://jira.internal/mcp
```

One line each, so this works unchanged in a `.env`, a systemd `EnvironmentFile`, a Compose
