...
**A restrictive default costs a round trip per tool call, and that is the trade.** Some providers run
their own agent loop and will only accept a denylist by name — and a name they were never shown cannot
be denied, so "everything except these" is not expressible upstream. A `deny` or `ask` default
therefore takes the whole tool loop back from the provider and runs it in Gridline: N calls where an
allow default takes one. Correct, and slower. Choose it where the scoping is worth the latency, not by
default.

From the Python SDK:

```python
harness = gridline.Harness(
    tools=[ledger],
    default=gridline.DENY,          # reaches every tool on the server, named or not
    allow=["read_ledger"],          # the exceptions
)
```

`default` is `allow` unless you say otherwise, and `ask=` / `deny=` are its exceptions the same way
`allow=` is a deny default's. **The SDK refuses an exception list that repeats the default** — `allow=`
beside an allow default — when you construct the harness, rather than silently doing nothing, because
the configuration somebody believes they wrote in that case is the opposite of the one they did. A
`.gridline` file writes the same block as plain `default` and `tools` keys and gets no such refusal, so
there the redundant entry simply has no effect.

## The shape

```python
chat = grid.session(agent="ops")
reply = await chat.send("close the ledger")
if reply.awaiting_approval:
    for request in reply.approvals:
        print(request.name, request.arguments)
    reply = await chat.resume({request.id: True for request in reply.approvals})
```

`resume` takes a mapping of **call id to a boolean**, so a denial is `False` — there is no separate
approve or deny call to reach for. The decision goes back on the conversation, which is what carries
it to whichever session actually stopped.
...
```python
answer = await chat.send("audit the ledger")
if answer.awaiting_approval:
    ...
    answer = await chat.resume({r.id: True for r in answer.approvals})
```
...
A reply that paused carries them already, in `reply.approvals`. A receiver that was told by a webhook
instead has no reply to read, so it fetches them from the control plane rather than the request path,
**with a credential that carries `content`** — the arguments are read out of the stored conversation,
so it is the same capability a transcript needs, and a `config`-only key is refused `403`:
...
They are deliberately **not** in the webhook body: an endpoint holds no capability, so it is given
nothing needing one. The `approval.required` event tells you *that* a decision is waiting and gives
you the session to look it up with.
