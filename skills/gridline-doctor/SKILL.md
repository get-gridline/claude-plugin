...
to a project. Establish this now rather than reporting fifteen permission errors, and read it
against what each thing below actually needs: **`config` reads configuration and does not write
it**, so a credential holding only `config` explains every write failure and no read failure.
Applying a whole project document is `deploy`; moving one agent, harness or connection, and
anything to do with credentials, webhooks or tunnels, is `admin`; reading a transcript is
`content`. See `gridline-keys`.
