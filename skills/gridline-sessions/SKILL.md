...
```python
await chat.queue("actually -- only January")   # from a web handler, a keystroke, anywhere
```
...
`await chat.inbox()` says what is waiting, what was delivered and what expired. It is the first
thing to call when something was queued and nothing happened. (Both of these are `await`ed because
`connect()` returns the asynchronous client unless you pass `sync=True` — see `references/sdk.md`.)
