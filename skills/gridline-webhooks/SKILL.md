...
subscribe_webhook(project, endpoint=…, events=["approval.required", "budget.exceeded"])
```
...
event = webhooks.unwrap(body, signature=header, secret=os.environ["GRIDLINE_WEBHOOK_SECRET"])
```
...
To get content, fetch it with a credential allowed to have it — `grid.approvals(...)` for a pending
approval, `GET /sessions/{id}/messages` for a transcript. The event carries the id to look it up with.
