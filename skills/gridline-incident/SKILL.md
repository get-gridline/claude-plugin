...
cost_report(project, by="model", days=1)
```
...
degraded responses rather than failed ones — `X-Route-Degraded` set, answers served by hop two.
...
|---|---|
| No fallback configured | Add a hop. Check the capability classes first — a chain that is refused when applied is not a fallback |
...
| Latency up, no errors | Check whether a slower fallback is serving. `X-Route-Model` on a sample of responses answers it |
