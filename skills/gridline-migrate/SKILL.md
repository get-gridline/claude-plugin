...
Tier one works from any language — it is an HTTP base URL. **Tier two is Python-only today**, so say
plainly that a Node or Go codebase stops at tier one, and what that costs (no approvals, no unified
reply shape). Do not imply an SDK exists that does not.
